...
impl PatchOffsets {
    ...
    const BUILD_IN_ROOMS: u64 =  0x18097D;
    
    const DIG_ROCK_FAST: u64 = 0x355A5B;
    
    const DRILL_ANYTHING: (u64, u64) = (0x355A48, 0x2478D28);
    
    const DRILL_MATERIAL: (u64, u64) = (0x355A53, 0x2478D98);
...
pub fn enable_drill_anything(base_address: u64) {
    unsafe {
        ...
        let _ = WriteProcessMemory(process, (base_address + PatchOffsets::DRILL_ANYTHING.0) as *const c_void, buffer.as_ptr().cast(), 2, None);
        ...
        let _ = VirtualProtect((base_address+PatchOffsets::DRILL_ANYTHING.1) as *const c_void, 0x2000, PAGE_PROTECTION_FLAGS(0x04), old.as_mut_ptr() as *mut PAGE_PROTECTION_FLAGS);
        
        let replacement_length: [u8; 1] = [new_text.len() as u8];
        let _ = WriteProcessMemory(process, (base_address + PatchOffsets::DRILL_ANYTHING.1) as *const c_void, replacement_length.as_ptr().cast(), 1, None);
        
        let _ = VirtualProtect((base_address+PatchOffsets::DRILL_ANYTHING.1) as *const c_void, 0x2000, PAGE_PROTECTION_FLAGS(0x02), old.as_mut_ptr() as *mut PAGE_PROTECTION_FLAGS);
    };
}
...
pub fn set_drill_material(base_address: u64, material: u8) {
    unsafe {
        ...
        let _ = WriteProcessMemory(process, (base_address + PatchOffsets::DRILL_MATERIAL.0) as *const c_void, buffer.as_ptr().cast(), 5, None);
        
        let mut old = Vec::with_capacity(20);
        let _ = VirtualProtect((base_address+PatchOffsets::DRILL_MATERIAL.1) as *const c_void, 0x2000, PAGE_PROTECTION_FLAGS(0x04), old.as_mut_ptr() as *mut PAGE_PROTECTION_FLAGS);
        ...
        let _ = WriteProcessMemory(process, (base_address + PatchOffsets::DRILL_MATERIAL.1) as *const c_void, replacement_length.as_ptr().cast(), 1, None);
        
        let _ = VirtualProtect((base_address+PatchOffsets::DRILL_MATERIAL.1) as *const c_void, 0x2000, PAGE_PROTECTION_FLAGS(0x02), old.as_mut_ptr() as *mut PAGE_PROTECTION_FLAGS);
    };
}
...
pub fn set_all_light(base_address: u64, light: bool) {
    unsafe {
        ...
        if light {
            ...
            let _ = WriteProcessMemory(process, (base_address + PatchOffsets::ALL_LIGHT.0) as *const c_void, lbuf1.as_ptr().cast(), 2, None);
            let _ = WriteProcessMemory(process, (base_address + PatchOffsets::ALL_LIGHT.1) as *const c_void, lbuf2.as_ptr().cast(), 4, None);
        } else {
...
pub fn init_patches(base_address: u64) {
    unsafe {
        ...
        if SETTINGS.read().unwrap().get::<bool>("patches.dig_rock_fast").unwrap() {
            ...
            let _ = WriteProcessMemory(process, (base_address + PatchOffsets::DIG_ROCK_FAST) as *const c_void, buffer.as_ptr().cast(), 5, None);
        }
